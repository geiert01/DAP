...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = NoopResetEnv(
            env, noop_max=30
        )  # wait a rand num of frames until env starts (max 30)
        env = MaxAndSkipEnv(
            env, skip=4
        )  # only consider every 4th frame (fix action for 4 frames)
        env = EpisodicLifeEnv(
            env
        )  # treats losing life as failing, but env continues until all lifes are gone
        if "FIRE" in env.unwrapped.get_action_meanings():
            env = FireResetEnv(
                env
            )  # if one needs to press FIRE to start the game, this is handled here
        env = ClipRewardEnv(
            env
        )  # don't adjust to reward size, just to good or bad: rewards = {-1, 0, 1}
        env = gym.wrappers.ResizeObservation(
            env, (84, 84)
        )  # smaller representation of image by interpolation
        env = gym.wrappers.GrayScaleObservation(env)  # convert to greyscale
        env = gym.wrappers.FrameStack(
            env, 4
        )  # use 4 frames as one input to get temporal information
...
def spectral_norm_conv(conv_layer, input_shape, n_iterations=20, device="cpu"):
    ...
    for _ in range(n_iterations):
        ...
        # Compute gradients
        grad_x = torch.autograd.grad(y_norm, x, retain_graph=True, create_graph=False)[
            0
        ]  # \frac{A^T A x_k}{||A x_k||}

        # Normalize the gradient to get the next iteration input
        x = grad_x / (
            grad_x.norm() + 1e-12
        )  # \frac{||A^T A x_k||}{||A x_k||} / \frac{A^T A x_k}{||A x_k||}
...
if __name__ == "__main__":
    ...
    for global_step in range(args.total_timesteps):
            ...
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    target_max, _ = target_network(data.next_observations).max(
                        dim=1
                    )  # qval of best act in next state
                    # = Q(S_{t+1}, A_{t+1})
        ...
        if global_step % 1000 == 0:
            lipschitz_constant = compute_lipschitz_constant(
                q_network, input_shape=(1, 4, 84, 84)
            )
            writer.add_scalar(
                "charts/lipschitz_constant", lipschitz_constant, global_step
            )
            print(
                f"Global step {global_step}: Lipschitz constant = {lipschitz_constant}"
            )
