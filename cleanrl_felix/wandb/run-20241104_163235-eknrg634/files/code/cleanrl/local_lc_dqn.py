...
def compute_local_lipschitz_constant(model, env, num_samples=100, epsilon=1e-2):
    ...
    obs_space_dim = np.array(env.single_observation_space.shape).prod()
    ...
    for sample in samples:
        ...
        perturbed_x = torch.clamp(
            perturbed_x,
            min=torch.tensor(env.single_observation_space.low, dtype=torch.float32),
            max=torch.tensor(env.single_observation_space.high, dtype=torch.float32)
        )
