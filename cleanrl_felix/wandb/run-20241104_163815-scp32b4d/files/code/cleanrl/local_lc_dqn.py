...
def compute_local_lipschitz_constant(model, env, num_samples=100, epsilon=1e-2):
    ...
    for sample in samples:
        x = torch.tensor(sample, dtype=torch.float32).unsqueeze(0).cpu()  # Move to CPU
        perturbed_x = x + epsilon * torch.randn_like(x)

        # Ensure perturbed_x stays within observation space bounds
        perturbed_x = torch.clamp(
            perturbed_x,
            min=torch.tensor(env.observation_space.low, dtype=torch.float32).cpu(),
            max=torch.tensor(env.observation_space.high, dtype=torch.float32).cpu()
        )

        # Forward pass through the network with weights on CPU
        original_output = model(x).cpu().detach()
        perturbed_output = model(perturbed_x).cpu().detach()
